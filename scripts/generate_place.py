...
@click.argument('network_type', default="drive")
def h3(place_path: str, out_dir: str, resolution: int, buffer: bool, network_type: str):
    # click.echo(place_path)
    place: gpd.GeoDataFrame = gpd.read_file(place_path, driver="GeoJSON")  # type: ignore
    generate_hexagons_for_place(place, resolution, out_dir, network_type, buffer)
...
def features(place_dir: str, network_type: str, resolution: int, buffered: bool):
    ...
    gpkg_path = Path(place_dir).joinpath(f"graph_{network_type}.gpkg")
    edges = gpd.read_file(gpkg_path, layer="edges")
    edges_with_hexagons = assign_hexagons_to_edges(edges, hexagons)  # type: ignore

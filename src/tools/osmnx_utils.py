...
def download_and_save_data_for_place(place_name: str, out_dir: Path, network_type: str, h3_resolutions: List[int], regions: Optional[List[str]]):
    ...
    if regions is not None:
        place = ox.geocode_to_gdf(regions).dissolve()
        place.display_name = place.display_name.apply(lambda x: ", ".join([place_name.split(",")[0]] + x.split(", ")[1:]))
    else:
        place = ox.geocode_to_gdf(place_name)
    place.to_file(out_dir / "place.geojson", driver="GeoJSON")

    G = ox.graph_from_polygon(place.iloc[0]["geometry"], network_type=network_type, retain_all=True)
